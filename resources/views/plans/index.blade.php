                ...
                <div class="bcard {{ $user->hasPlanExpired() ? 'border-red-300 bg-red-50' : '' }}">
                    ...
                    @if($user->hasPlanExpired())
                        <div class="bg-red-100 border border-red-300 rounded-lg p-3 mb-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-medium text-red-800">Plan Expired</span>
                            </div>
                            <p class="text-red-700 text-sm mt-1">
                                Your features are now limited to Free plan. You can renew your current plan, upgrade to a new plan, or switch to Free plan.
                            </p>
                        </div>
                    @endif
                    ...
                    <div class="space-y-2 tm-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold">${{ number_format($currentPlan->price ?? 0, 2) }}/month</span>
                        </div>

                        <div class="flex justify-between">
                            <span>{{ $user->hasPlanExpired() ? 'Expired on' : 'Ends on' }}</span>
                            <span class="font-semibold {{ $user->hasPlanExpired() ? 'text-red-600' : '' }}">
                                {{ $user->plan_expires_at?->format('M d, Y') ?? 'Unlimited' }}
                            </span>
                        </div>
                        ...
                        @if($user->hasPlanExpired())
                            <div class="flex justify-between">
                                ...
                                <span class="font-semibold text-orange-600">Free Plan Limits</span>
                            </div>
                        @endif
                ...
                @if($user->hasPlanExpired())
                    <div class="bcard bg-red-50 border-red-200">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h4 class="font-medium text-red-800">Plan Expired!</h4>
                                <p class="text-red-700 text-sm">Your plan expired {{ abs($user->getDaysUntilExpiration()) }} days ago. Renew to continue using premium features.</p>
                            </div>
                        </div>
                    </div>
                @elseif($user->isPlanExpiringSoon())
                    <div class="bcard bg-yellow-50 border-yellow-200">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h4 class="font-medium text-yellow-800">Plan Expiring Soon!</h4>
                                <p class="text-yellow-700 text-sm">Your plan expires in {{ $user->getDaysUntilExpiration() }} days. Consider renewing to avoid interruption.</p>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                @if(!$currentPlan->isFree())
                    ...
                    <div class="bcard">
                        <h4 class="font-medium mb-3">
                            {{ $user->hasPlanExpired() ? 'Plan Options' : 'Quick Actions' }}
                        </h4>
                        ...
                        @if($user->hasPlanExpired())
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                ...
                                <button class="button outline" data-bs-toggle="modal" data-bs-target="#downgrade1">
                                    <svg class="size-4 mr-2" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <p class="text-sm text-gray-600 mt-3">
                                Choose to renew your current plan, upgrade to a different plan, or switch to the free plan.
                            </p>
                        @else
                            <div class="flex gap-3">
                                ...
                                <button class="button outline" data-bs-toggle="modal" data-bs-target="#downgradeModal">
                                    <svg class="size-4 mr-2" viewBox="0 0 20 20" fill="currentColor">
        ...
        @else
            <div class="bcard bg-blue-50 border-blue-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h4 class="font-medium text-blue-800">No Active Plan</h4>
                        <p class="text-blue-700 text-sm">You don't have an active plan. Choose a plan below to get started!</p>
                    </div>
                </div>
            </div>
        @endif

        <h2 class="t-lg">Available Plans</h2>
                ...
                <div class="bcard {{ $currentPlan && $user->plan_id == $plan->id ? 'ring-2 ring-green-500' : '' }}">
                    <div class="flex items-center justify-between gap-x-4">
                        ...
                        @if($currentPlan && $user->plan_id == $plan->id)
                            <div class="badge green">Current</div>
                        @endif
                    </div>
                    <p class="flex items-baseline gap-x-1 mb-6">
                        @if($plan->isFree())
                            <span class="text-xl font-semibold tracking-tight text-green-600">Free</span>
                        @else
                    ...
                    @if ($plan->id == $user->plan_id)
                        <button class="button outline" disabled>Current Plan</button>
                    @else
                        @if($plan->isFree())
                    ...
                    <ul class="mt-6 space-y-2 text-sm/6 tm">
                        <li class="flex gap-x-3">
                            ...
                            {{ $plan->formatted_storage_limit }} of storage
                        </li>
                        <li class="flex gap-x-3">
                            <svg class="h-6 w-5 flex-none text-blue-600" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                <path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 0 1 .143 1.052l-8 10.5a.75.75 0 0 1-1.127.075l-4.5-4.5a.75.75 0 0 1 1.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 0 1 1.05-.143Z" clip-rule="evenodd" />
                            </svg>
                            {{ $plan->formatted_file_size_limit }} per file
                        </li>
                        <li class="flex gap-x-3">
                            <svg class="h-6 w-5 flex-none text-blue-600" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                <path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 0 1 .143 1.052l-8 10.5a.75.75 0 0 1-1.127.075l-4.5-4.5a.75.75 0 0 1 1.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 0 1 1.05-.143Z" clip-rule="evenodd" />
                        ...
                        <div class="modal-dialog">
                            <div class="content">
                                <div class="panel">
                                    ...
                                    <p class="tm-sm">Payment method (30 days)</p>
                                    <ul class="menu-list mb-6" role="list">
                                        <li>
                                            <button class="w-full" data-id="{{ $plan->id }}" data-method="balance" data-duration="30">
                                                ...
                                                <span>Balance ({{ currencyFormat($user->balance) }})</span>
                                            </button>
                                        </li>
                                ...
                                <div class="panel">
                                    ...
                                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                                        ...
                                        <p class="text-yellow-700 text-sm mt-2">
                                            @if($plan->isFree())
                                                Downgrading to the free plan will immediately deactivate your current plan. You will lose access to premium features.
                                            @else
                                                Downgrading will change your plan to {{ $plan->name }} (${{ number_format($plan->price, 2) }}/month). Some features may be limited.
                                            @endif
                                        </p>
                                    </div>
                                    <p class="tm-sm mb-4">Are you sure you want to downgrade?</p>
                                    <div class="flex gap-3">
                                        <button type="button" class="button secondary" data-bs-dismiss="modal">Cancel</button>
            ...
            @if(!$userAutoRenew)
                <div class="modal fade renew" tabindex="-1" aria-hidden="true" id="renewModal">
                    <div class="modal-dialog">
                        <div class="content">
                            <div class="panel">
                                <h3 class="text-lg font-semibold mb-4">Renew {{ $currentPlan->name }}</h3>
                                ...
                                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="font-medium text-blue-800">Plan Extension</span>
                                    </div>
                                    <p class="text-blue-700 text-sm mt-2">
                                        @if($user->hasPlanExpired())
                                            Your plan expired {{ abs($user->getDaysUntilExpiration()) }} days ago.
                                            Renewal will start from today and last for 30 days.
                                        @else
                                            Your plan expires {{ $user->plan_expires_at->format('M d, Y') }}.
                                            Renewal will extend your plan by 30 days from the current expiration date.
                                        @endif
                                    </p>
                                </div>

                                <p class="tm-sm mb-4">Choose payment method (30 days - ${{ number_format($currentPlan->price, 2) }})</p>
                                <ul class="menu-list mb-6" role="list">
                                    <li>
                                        <button class="w-full" data-id="{{ $currentPlan->id }}" data-method="balance" data-duration="30">
                                            <div class="flex justify-between items-center w-full">
                                                <span>Balance</span>
                                                <span>{{ currencyFormat($user->balance) }}</span>
                                            </div>
                                        </button>
                                    </li>
