...
<div class="row row-deck">
    <div class="col-12">
        <form class="form card" action="{{ route('admin.pages.update', $page->id) }}" method="POST" enctype="multipart/form-data">
            @method('PUT')
            @csrf
                ...
                <div class="row">
                    <div class="col-12 col-md-8">
                        <div class="mb-3">
                            ...
                            <input type="text" id="post-title" name="title" value="{{ $page->title }}" placeholder="Enter post title" class="form-control">
                        </div>
                        <div class="mb-3">
                            <label class="form-label required" for="post-slug">Slug</label>
                            <input type="text" id="post-slug" name="slug" value="{{ $page->slug }}" placeholder="Enter post slug" class="form-control">
                        </div>
                        <div class="mb-3">
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" id="status" class="form-control">
                                ...
                                <option value="public" @selected($page->status == 'public')>Công khai</option>
                                <option value="private" @selected($page->status == 'private')>Riêng tư</option>
                                <option value="draft" @selected($page->status == 'draft')>Nháp</option>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="meta_title">Meta title</label>
                            <input type="text" id="meta_title" name="meta_title" placeholder="Enter meta_title" class="form-control" value="{{ $page->meta_title }}">
                        </div>

                        <div class="mb-3">
                            <label class="form-label" for="meta_description">Meta description</label>
                            <input type="text" id="meta_description" name="meta_description" placeholder="Enter meta_description" class="form-control" value="{{ $page->meta_description }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="meta_keywords">Meta keywords</label>
                            <textarea type="text" id="meta_keywords" name="meta_keywords" placeholder="Enter meta_keywords" class="form-control">{{ $page->meta_keywords }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
