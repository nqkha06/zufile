...
@section('content')
    <form action="{{ route('admin.note_levels.update', $level->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="row">
            ...
            <div class="col-md-3 gap-3 d-flex flex-column-reverse flex-md-column mb-md-0 mb-5">
                ...
                <div class="card meta-boxes mb-3">
                    ...
                    <div class="card-body">
                        <input type="text" name="lang" value="{{ $lang?->code ?? config('app.DEFAULT_LANG_ADMIN') }}"
                            hidden>
                        <div id="list-others-language">
                            @foreach (Language::getSupportedLanguages()->where('code', '!=', $lang?->code ?? config('app.DEFAULT_LANG_ADMIN'))
                                as $lang)
                                <a class="gap-2 d-flex align-items-center text-decoration-none"
                                    ...
                                    <img src="{{ asset('backend/media/flags/' . $lang->flag . '.svg') }}"
                                        ...
                                        alt="{{ $lang->name }} flag">
                                    <span>{{ $lang->name }} <svg class="icon" xmlns="http://www.w3.org/2000/svg"
