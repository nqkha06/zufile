...
<div class="row row-deck">
    <div class="col-12">
        <form class="form card" action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
            @method('PUT')
            @csrf
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" id="status" class="form-control">
                                ...
                                <option value="public" @selected($post->status == 'public')>Công khai</option>
                                <option value="private" @selected($post->status == 'private')>Riêng tư</option>
                                <option value="draft" @selected($post->status == 'draft')>Nháp</option>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="meta_title">Meta title</label>
                            <input type="text" id="meta_title" name="meta_title" placeholder="Enter meta_title" class="form-control" value="{{ $post->meta_title }}">
                        </div>

                        <div class="mb-3">
                            <label class="form-label" for="meta_description">Meta description</label>
                            <input type="text" id="meta_description" name="meta_description" placeholder="Enter meta_description" class="form-control" value="{{ $post->meta_description }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="meta_keywords">Meta keywords</label>
                            <textarea type="text" id="meta_keywords" name="meta_keywords" placeholder="Enter meta_keywords" class="form-control">{{ $post->meta_keywords }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
